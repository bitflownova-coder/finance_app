...
class ReceiptOcrHelper @Inject constructor(
    ...
    suspend fun extractAmountFromReceipt(imageFile: File): Double? {
        return try {
            // Load bitmap at OCR-optimized size (prevents canvas error)
            val bitmap = imageStorageHelper.loadBitmapForOcr(imageFile.absolutePath)
            if (bitmap == null) {
                Log.e(TAG, "Failed to load bitmap for OCR")
                return null
            }
            ...
            imageStorageHelper.recycleBitmap(bitmap)
