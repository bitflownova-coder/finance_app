...
class UserRepositoryImpl @Inject constructor(
            ...
            try {
                ...
                if (existingUser != null) {
                    return@withContext Result.Error(
                        Exception("This phone number is already registered. Please use a different number or clear app data to start fresh.")
                    )
                }
    ...
    override suspend fun updatePin(userId: Long, newPinHash: String): Result<Unit> =
        withContext(ioDispatcher) {
            try {
                userDao.updatePin(userId, newPinHash)
                Result.Success(Unit)
            } catch (e: Exception) {
                Result.Error(e)
            }
