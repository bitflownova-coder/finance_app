...
class ReportRepositoryImpl @Inject constructor(
    ...
    private suspend fun getTransactionList(userId: Long, startDate: Long, endDate: Long): List<Transaction> {
        val transactionEntities = mutableListOf<com.financemanager.app.data.local.entities.TransactionEntity>()
        transactionDao.getTransactionsByDateRange(userId, startDate, endDate).collect { entities ->
            transactionEntities.clear()
            transactionEntities.addAll(entities)
        }
    ...
    override suspend fun getBudgetComparison(userId: Long, dateRange: DateRange): List<BudgetComparison> = withContext(Dispatchers.IO) {
        val budgets = budgetDao.getCurrentBudgets(userId, dateRange.startDate)
        val budgetEntities = mutableListOf<com.financemanager.app.data.local.entities.BudgetEntity>()
        
        budgets.collect { entities ->
            budgetEntities.clear()
            budgetEntities.addAll(entities)
        }
